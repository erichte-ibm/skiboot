			       ...
			       struct list_head *update_bank)
{
	...
	list_for_each(update_bank, var, link) {
		...
		rc = process_update(var, &newesl, &neweslsize, &timestamp, &staging_bank,
				    tsvar->data);
		if (rc) {
		 ...
		 */
		rc = update_variable_in_bank(var, newesl, neweslsize, &staging_bank);
		if (rc) {
		...
		rc = update_timestamp(var->key, &timestamp, tsvar->data);
		if (rc) {
