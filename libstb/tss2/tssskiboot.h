...
int tss_nv_read_public(TPMI_RH_NV_INDEX nv_index, TPMS_NV_PUBLIC *nv_public,
		       TPM2B_NAME *nv_name);
int tss_nv_read(TPMI_RH_NV_INDEX nv_index, unsigned char *buffer,
		     size_t buffer_size,  uint16_t offset);
int tss_nv_write(TPMI_RH_NV_INDEX nv_index, unsigned char *buffer,
		      size_t buffer_size, uint16_t offset);
...
int tss_get_random_number(char *buffer, uint16_t bytes_requested);
